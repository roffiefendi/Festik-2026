...
export function SectionCountdown({ countdown }: CountdownProps) {
  const targetDate = useMemo(() => {
    if (!countdown?.eventDate || !countdown?.eventTime)
      return new Date("2026-06-13T14:00:00+07:00");
    const timezone = countdown.timezone || "+07:00";
    return sanitizeDate(
      countdown.eventDate,
      countdown.eventTime,
      timezone === "Asia/Jakarta" ? "+07:00" : timezone,
    );
  }, [countdown]);
  ...
  useEffect(() => {
    const interval = setInterval(
      () => setState(formatCountdown(targetDate)),
      1000,
    );
  ...
  return (
    <section
      id="countdown"
      className="section-container relative overflow-hidden px-6 py-24 sm:px-8 lg:px-12"
    >
      ...
      <div className="mx-auto max-w-6xl rounded-3xl border border-white/15 bg-gradient-to-br from-white/8 to-white/[0.03] p-8 sm:p-12 shadow-elevated backdrop-blur-2xl relative z-10">
        <div className="grid gap-8 lg:grid-cols-[1.2fr_1fr]">
          <motion.div
            ...
            className="space-y-6"
          >
            <span className="inline-block bg-gradient-cyan px-4 py-2 rounded-full text-xs uppercase tracking-[0.32em] text-cyan/80 font-medium">
              Festival Countdown
            </span>
            <h2 className="font-serif text-4xl leading-tight text-cream sm:text-5xl">
              Experience the opening ceremony in cinematic time.
            </h2>
            <p className="max-w-xl text-slate-300 leading-relaxed">
              The Festival Teknik countdown is controlled from Sanity CMS. When
              the clock expires, the platform automatically celebrates the
              moment.
            </p>
          </motion.div>
          <motion.div
          ...
          >
            {state.expired ? (
              <div className="col-span-2 flex min-h-[220px] flex-col items-center justify-center rounded-2xl border border-white/15 bg-gradient-to-br from-magenta/10 to-purple/5 p-8 text-center backdrop-blur-xl\">
                <span className="text-xs uppercase tracking-[0.36em] text-sage/80 font-medium\">
                  STATUS
                </span>
                <p className="mt-4 text-3xl font-semibold text-cream\">
              ...
              ["days", "hours", "minutes", "seconds"].map((label, idx) => (
                ...
                >
                  ...
                  <span className="mt-3 block text-xs uppercase tracking-[0.32em] text-slate-300 group-hover:text-slate-200 transition-colors\">
                    {label}
                  </span>
                </motion.div>
              ))
